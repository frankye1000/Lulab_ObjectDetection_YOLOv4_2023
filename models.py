...
class Yolov4(object):
    ...
    def build_model(self, load_pretrained=True):
        ...
        self.training_model.compile(optimizer = optimizers.Adam(lr=1e-3), 
                                    ...
                                    # metrics   = [tf.keras.metrics.AUC(curve='PR')]  #curve='PR'
                                    )
    ...
    def load_weights(self, path):
        self.yolo_model.load_weights(path)
    ...
    def fit(self, train_data_gen, epochs, val_data_gen=None, initial_epoch=0, callbacks=None):
        history = self.training_model.fit(train_data_gen,
    ...
    # raw_img: RGB
    def predict_img(self, raw_img, random_color=True, plot_img=True, figsize=(10, 10), show_text=True, box_num=1, return_output=False):
        ...
        # 限制一張圖片只畫box_num個方框
        # detections = detections[:box_num]
        # 把class='not'方框去除
        # detections = detections[detections['class_name']!='not']
    ...
    def predict(self, img_path, random_color=True, plot_img=True, figsize=(10, 10), show_text=True, box_num=1, return_output=True):
        raw_img = cv2.imread(img_path)[:, :, ::-1]
        return self.predict_img(raw_img, random_color, plot_img, figsize, show_text, box_num, return_output)
